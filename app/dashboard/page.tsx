...
interface TeamStats {
  ...
  sizeMap:    Record<string, number>   // size_category → car count
  serviceMap: Record<string, { count: number; revenue: number }> // all services (base + addon)
}
...
export default function DashboardPage() {
  ...
  function priceFor(svcName: string, sizeCat: string): number {
    const match = servicePrices.find((sp) => sp.service_name === svcName && sp.size_category === sizeCat)
  ...
  teams.forEach((t) => { teamStats[t] = { cars: 0, revenue: 0, addons: 0, addonMap: {}, sizeMap: {}, serviceMap: {} } })

  txRange.forEach((t) => {
    if (!t.team) return
    const key = t.team
    if (!teamStats[key]) teamStats[key] = { cars: 0, revenue: 0, addons: 0, addonMap: {}, sizeMap: {}, serviceMap: {} }
          ...
          <div className="rounded-2xl bg-white p-5 shadow-sm">
            {sortedTeams.length === 0 || sortedTeams.every((t) => teamStats[t].cars === 0) ? (
              ...
              <div className="space-y-3">
                {sortedTeams.map((team, i) => {
                  ...
                  const totalServiceRevenue = serviceEntries.reduce((s, [, v]) => s + v.revenue, 0)
                              ...
                              addonEntries.length === 0 ? (
                                ...
                                <>
                                  ...
                                  <div className="mt-2 flex justify-between border-t border-gray-100 pt-2 text-xs font-semibold">
                                    ...
                                    <span style={{ color }}>{formatPHP(totalAddonRevenue)}</span>
                                  </div>
                                </>
                              )
                            )}
                              ...
                              serviceEntries.length === 0 ? (
                                ...
                                <>
                                  ...
                                  <div className="mt-2 flex justify-between border-t border-gray-100 pt-2 text-xs font-semibold">
                                    ...
                                    <span style={{ color }}>{formatPHP(totalServiceRevenue)}</span>
                                  </div>
                                </>
                              )
                            )}
            ...
            <div className="mt-4 rounded-xl bg-gray-50 px-3 py-2">
              <p className="text-[10px] text-gray-400">
                <strong>Add-ons</strong> = any service beyond Basic Wash or Body Wash. Click <strong>Details ▼</strong> on any team to see add-ons, size breakdown, or full service totals.
              </p>
            </div>
          </div>
        </section>
