  ...
  return (
    <div className="flex flex-col items-center gap-4 sm:flex-row sm:items-start">
      ...
      <div className="flex flex-1 flex-wrap gap-x-6 gap-y-2">
        {slices.map(([cat, amt]) => (
          <div key={cat} className="flex cursor-pointer items-center gap-2 text-sm"
            onMouseEnter={() => setHovered(cat)} onMouseLeave={() => setHovered(null)}
...
export default function PnLPage() {
  ...
  async function handleExpenseSubmit(e: React.FormEvent) {
    ...
    const { error } = await supabase.from('expenses').insert({
      date: form.date, assignee: normAssignee || null, description: normDesc,
      category: form.category, amount: parseFloat(form.amount), payment_type: form.payment_type, notes: form.notes,
    })
    setFormSaving(false)
          ...
          <div className="space-y-8">
            ...
            <div className="grid gap-8 lg:grid-cols-2">
              ...
              <section className="rounded-2xl bg-white p-6 shadow-sm">
                ...
                <p className="mb-4 text-xs text-gray-400">{rangeLen > 31 ? 'Grouped by week' : 'Day-by-day spending per category'}</p>
                  ...
                  <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
                    ...
                    {form.category !== 'Salary' && (
                      <div>
                        ...
                        <input type="text" name="description" value={form.description} onChange={handleFormChange} placeholder="e.g. Gas, Food, Cash Advance" className={inputCls} />
                      </div>
                    )}
                    <div>
